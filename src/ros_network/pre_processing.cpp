...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    ros::Subscriber sub_camera = nh.subscribe("/visual_cloud", 10, visual_cloud_cb);
    ros::Publisher pub_visual_cloud = nh.advertise<sensor_msgs::PointCloud2>("/cloud_preprocessed", 10);
    float x_min = -0.25, x_max = 0.15;
    float y_min = -0.60, y_max = -0.10;
    float z_min = 0.10, z_max = 0.30;
    ...
    while(ros::ok())
    {
        ...
        if(visual_cloud->points.size() != 0)
        {
            ...
            std::cout << std::endl << "PRE-PROCESSING" << std::endl;
            ...
            pcl::io::savePCDFile("/home/workstation2/pcl_preprocessata.pcd", *visual_cloud);
