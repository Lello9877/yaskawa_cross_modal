...
private:
    ...
    // using new keyword
    geometry_msgs::Point *array = NULL;
    // Size of array
    int size;
...
public:
    ...
    DynamicArray()
    {
        capacity = 1;
        size = 0;
        array = new geometry_msgs::Point[capacity];
    }
    ...
    void push_back(geometry_msgs::Point value)
    {
    ...
    void growArray()
    {
        ...
        geometry_msgs::Point* temp = new geometry_msgs::Point[capacity * 2];
    ...
    void shrinkArray()
    {
        ...
        capacity = size;
        geometry_msgs::Point* temp = new geometry_msgs::Point[capacity];
    ...
    int search(geometry_msgs::Point key)
    {
        for (int i = 0; i < size; i++) {
            if (array[i] == key) {
    ...
    void insertAt(int index, geometry_msgs::Point value)
    {
        // check is array having size to store element or
        // not
        if (size == capacity) {
